...
| Range | Width | Policy |
| --- | --- | --- |
| Mobile | `< 768px` | 기본 스타일. 세로 배치, 컴팩트 타이포, 터치 중심 UI |
| Tablet | `768px - 1023px` | 기본 모바일 계열 레이아웃 유지. 필요할 때만 `md:`로 보정 |
| Desktop | `>= 1024px` | `lg:` 스타일 적용. 데스크톱 메뉴, 그리드, desktop typography 사용 |

현재 주요 레이아웃 전환점은 `1024px`입니다. `/diagnosis/info`에서는 `1023px 이하`를 모바일/태블릿 레이아웃으로 보고, `1024px 이상`부터 데스크톱 레이아웃을 적용합니다.
